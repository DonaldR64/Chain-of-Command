const Main = (() => {
    ...
    class Team {
        constructor(id) {
            ...
            this.nation = nation;
            let player = (state.CoC2.nations.indexOf(nation));
            if (player === -1) {
                if (nation === "Neutral") {
                    player = 2
                } else {
                    state.CoC2.nations.push(nation);
                    player = state.CoC2.nations.length - 1;
                }
            }
            this.player = player;
            this.token = token;
    ...
    const RemoveLines = (which = ["LOS","Deploy"]) => {
        _.each(which,lines => {
            let array;
            if (lines === "LOS") {
                array = state.CoC2.losLines;
            }
            if (lines === "Deploy") {
                array = state.CoC2.deployLines;
            }
            if (array) {
                for (let i=0;i<array.length;i++) {
                    let id = array[i];
    ...
    const DrawLine = (set,colour = "#ff0000",type = "Deploy") => {
        ...
        if(page) {
            ...
            if (line) {
                toFront(line);
                if (type === "LOS") {
                    state.CoC2.losLines.push(line.get("id"))
                } else {
                    state.CoC2.deployLines.push(line.get("id"));
                }
            }
        }
    }
    ...
    const RollDice = (msg) => {
        ...
        if ((!id || !team) && playerID) {
            nation = state.CoC2.players[playerID];
            player = (state.CoC2.nations[0] === nation) ? 0:1;
        }

        if (!state.CoC2.players[playerID] || state.CoC2.players[playerID] === undefined) {
            if (nation !== "Neutral") {    
                state.CoC2.players[playerID] = nation;
            } else {
